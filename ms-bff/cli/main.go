...
func main() {
	...
	if err != nil {
		log.Error().Err(err).Msg("Error in creating a client for ms-highscore")
	}
	gameEngineClient, err := bff.NewGrpcGameEngineServiceClient(*grpcAddressGameEngine)
	if err != nil {
		log.Error().Err(err).Msg("Error in creating a client for ms-game-engine")
	}
